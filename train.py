...
# tf.flags.DEFINE_string("log_dir", default='log')

tf.flags.DEFINE_integer("num_checkpoints", 1,
                        """Number of checkpoints to store (default: 1)""")
tf.flags.DEFINE_integer("num_epochs", default=8,
...
def evaluate(session, dataset, model, loss, accuracy, pre, summary_op=None,city='None'):
    sum_loss = 0.0
    sum_acc = 0.0
    example_count = 0
    global valid_step
    ...
    for reviews, images, labels in dataset:
        ...
        _loss, _acc, _pre = session.run([loss, accuracy, pre], feed_dict=feed_dict)
    ...
    if city!='None':
        df=pd.DataFrame(pre_list)
        df.to_csv('data/predict/{}-acc={}.csv'.format(city,avg_acc))
...
def train(session, data_reader, model, train_op, loss, accuracy, pre, summary_op):
    for reviews, images, labels in data_reader.read_train_set(batch_size=FLAGS.batch_size):
        step, _, _loss, _acc, _pre = session.run([model.global_step, train_op, loss, accuracy, pre],
                                                 feed_dict=model.get_feed_dict(reviews, images, labels,
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        while epoch < FLAGS.num_epochs:
            ...
            train(sess, data_reader, model, train_op, loss, accuracy, pre, summary_op)
            ...
            valid_loss, valid_acc = evaluate(sess, data_reader.read_valid_set(batch_size=FLAGS.batch_size),
                                             model, loss, accuracy, pre, summary_op)
            ...
            if valid_acc > best_acc:
                ...
                print('=> Testing')
                result_file = open(
                  os.path.join('log', 'loss={:.4f},acc={:.4f},epoch={}'.format(valid_loss, valid_acc, epoch)), 'w')
                test(sess, data_reader, model, loss, accuracy, epoch,pre, result_file)
